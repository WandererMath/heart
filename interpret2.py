...
model.load_state_dict(torch.load(f'model{NTH_PLAN}.pth'))
model.eval()  # Set the model to evaluation mode


def get_grad_importance(i):
...
results=np.array([get_grad_importance(i) for i in range(n_sample)])
#gradient_importance=results.mean(axis=0)
with open(f"interpret{NTH_PLAN}.pkl", 'wb') as f:
